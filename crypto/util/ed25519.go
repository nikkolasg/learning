...
type Ed25519ReducedPrivate [ReducedPrivateKeySize]byte
...
// Does the sha512 thing + the pruning
...
func privateToDigest(priv []byte) []byte {
...
// take the module of the private key and extend with the public key
func ModuleExtend(suite abstract.Suite, priKey, public []byte) []byte {
...
func ReducedScalarToExtended(secret, public []byte) []byte {
	...
	copy(extended[32:], public)
	return extended[:]
}

func Abstract2Hex(s abstract.Marshaling) string {
	var buff []byte
	var err error
	switch s.(type) {
...
func GeEd255192Abstract(suite abstract.Suite, pubKey []byte) abstract.Point {
	point := suite.Point()
	...
	if !bytes.Equal(pubKey, buff) {
		panic("Not equal")
	}
	return point
}
func SliceToInt(suite abstract.Suite, scalar []byte) *nist.Int {
	rReduced := nist.NewInt(0, &primeOrder.V)
	rReduced.SetLittleEndian(scalar)
	return rReduced
}
